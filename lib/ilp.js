...
class Ilp {
  ...
  async sendMoney ({ ilpAmount, connection }) {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        stream.once('error', () => {
          resolve({ success: false, remainingAmount: ilpAmount - state.totalSent })
        })
  ...
  async sendIlpPayment ({ amount, pointer }) {
    ...
    let { success, remainingAmount } = await this.sendMoney({ ilpAmount: ilpScaledAmount, connection })
    ...
    return { success: !!success, remainingAmount: remainingAmount || 0 }
  }
}
