...
class Ilp {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        stream.once('error', (err) => {
          resolve({ success: false, remainingAmount: ilpAmount - state.totalSent, error: err.message })
        })

        this.log.info('Setting stream max to specified amount (%d)', ilpAmount)
        stream.setSendMax(ilpAmount)
      } catch (e) {
        resolve({ success: false, remainingAmount: ilpAmount - state.totalSent, error: e.message })
      }
    })
  }
  ...
  async sendIlpPayment ({ amount, pointer }) {
    ...
    if (error && remainingAmount === ilpScaledAmount) {
      throw new Error('ERROR: Encountered error when sending money via ILP: ', error)
    }

    // Simply log error, still continue noting the remaining amount but just log error for debugging purposes
    if (error) {
      this.log.info('ERROR: Encountered error when sending money via ILP: ', error)
    }
    ...
    return { success, remainingAmount: remainingAmount || 0 }
  }
}
